...
Here we can conclude that the SARIMAX model with 6 exogenous variables and `(3,0,2) (2, 1, 1) [7] with intercept` has best understood the underlying trend of the daily COVID cases. Looking at the literature though, it would appear as if RNNs with LSTM's provide an even more accurate Covid Case count forcast with a sMAPE of 5%. I would like to follow up with developing my own RNN model to reproduce this result and explore if the SIRD model parameters contribute more accuracy to its performance.  
